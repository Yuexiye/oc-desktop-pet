...
class PhoneActivityPerception:
    ...
    def add_activity(self, app_name: str, event: str = 'switch'):
        ...
        # 桥接到全局事件总线：任务系统据此推进 daily:phone 等
        # 注意：EventBus.emit 首参名为 event，故数据键用 action 而非 event 避免冲突
        EventBus.emit("phone_event", target=app_name, category=category, action=event)
