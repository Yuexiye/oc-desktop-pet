...
from core.event_bus import EventBus as GlobalEventBus
...
class MultiPetBridge:
    ...
    def _dispatch_loop(self) -> None:
        ...
        while self._dispatcher_running:
            ...
            # 桥接到全局事件总线：任务系统据此推进 multi_pet 相关任务。
            # 在分发后转发，保证每次多宠事件（含无内部订阅者时）都被任务系统感知。
            try:
                GlobalEventBus.emit(
                    "multi_pet_event",
                ...
                logger.exception("[MultiPetBridge] 转发 multi_pet_event 失败")
