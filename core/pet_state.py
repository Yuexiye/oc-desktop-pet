...
class PetStateManager:
    ...
    def __init__(
        ...
        self._attr_buckets: dict[str, int] = {}  # 属性变化事件节流：上次广播的 5 点桶
    ...
    def tick(self, dt_seconds: Optional[float] = None):
        ...
        # 属性变化事件在锁外发：与 mode 回调一致，避免总线派发时持锁
        self._emit_attribute_changes()
    ...
    # -------------------------------------------------------------------------
    # 属性变化事件（任务系统 attribute 类条件的数据源）
    # -------------------------------------------------------------------------
    def _emit_attribute_changes(self):
        """属性按 5 点桶变化时才发 attribute_changed，避免每秒刷屏。

        阈值类任务（如 mood>=80）会在跨桶时得到一次真实值广播；
        衰减天然让属性持续移动，故不会"已满足却永不完成"。
        payload: attribute=<属性名>, value=<当前值(float)>
        """
        attrs = ("health", "stamina", "hunger", "thirst", "mood", "likability", "energy")
        try:
        ...
        for attr in attrs:
            try:
                val = getattr(self.save, attr, None)
                if val is None:
                    continue
                bucket = int(val // 5)
                if self._attr_buckets.get(attr) != bucket:
                    self._attr_buckets[attr] = bucket
                    EventBus.emit("attribute_changed", attribute=attr, value=round(float(val), 2))
            except Exception:
                logger.debug("emit attribute_changed failed: %s", attr, exc_info=True)
