...
class PetSave(BaseModel):
    ...
    save_version: int = 2  # v2: 新增任务系统字段（gacha_energy 等），旧档靠默认值自动补全

    # ---- 当前模式（由 cal_mode() 写入）----
    mode: ModeType = ModeType.NORMAL

    # ---- 任务系统扩展（03 成长计划；字段命名见 oc-pet-矛盾对照.md 偏差说明）----
    # 双货币：通用货币复用现有 money 字段（03 的 credits 在此映射为 money），不另起 credits。
    # 盲盒能量独立命名为 gacha_energy：PetSave 已有 energy(精力，0-100 属性)，
    #   若复用 energy 名会直接覆盖精力属性，故改名以避免冲突。
    gacha_energy: float = 0.0           # 能量（盲盒专用货币）
    gacha_energy_max: float = 100.0     # 能量上限（03 §5.1：100≈10 抽）
    gacha_tickets: int = 0              # 免费盲盒券
    total_gacha_energy_earned: float = 0.0  # 累计获得能量（统计）
    badges: list = Field(default_factory=list)  # 已获得徽章 ID
...
class PetSaveManager:
    ...
    def __init__(self, save_path: str):
        ...
        # 升级回调钩子（由 pet.py 注入，用于解耦"升级事件"与保存层）。
        # 签名: on_level_up(old_level: int, new_level: int) -> None
        # 保存层保持纯净（无 Qt / 无事件总线依赖），事件发射交给上层异步处理。
    ...
    def level_up(self) -> None:
        ...
        # 解耦：升级事件交给上层异步发射（避免奖励结算重入 add_exp 递归）。
        # 钩子由 pet.py 注入，内部用 QTimer 延迟到事件循环空闲时发射。
        if self.on_level_up is not None:
            try:
                self.on_level_up(old_level, s.level)
            except Exception:
                pass
